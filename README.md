...
Each place is decided on its own: a journal can ask for the number only at registration, only
in the contributor form, in both, or in neither.
...
is free to write its own form: the `id` and the classes of the core may not be there. Since
1.3.0.0 the field is anchored on where the form **posts to** (`…/user/register`), which no theme
can change, and lands with the other fields — at the end of the identity block of the core, or
just before the control that sends the form. Before that, a journal with a theme of its own
saw no field at all, and nothing said why.
...
Cada lugar é decidido separadamente: a revista pode pedir o número só no cadastro, só na
autoria, nos dois ou em nenhum.

**Onde o campo entra na página de cadastro.** A página é do tema, e o tema pode escrever o
próprio formulário: o `id` e as classes do núcleo podem não existir. Desde a 1.3.0.0 o campo é
ancorado em **para onde o formulário posta** (`…/user/register`), que nenhum tema muda, e entra
junto dos outros campos — no fim do bloco de identificação do núcleo ou logo antes do botão que
envia o formulário. Antes disso, revista com tema próprio simplesmente não via o campo, e nada
dizia por quê.
